...
int main( int argc, char *argv[] )
{
	...
	if (help == true)
	{
	    printf("sega 0.3 enigma compressor/decompressor\nUsage: enicmp [-x|--extract {pointer}] [-v|--verbose] [-p|--padding] {input_filename} {output_filename}\n\n-x,--extract  extract from {pointer} address in file.\n-v,--verbose  display verbose output.\n-p,--padding  add or remove padding.\n");
	    return 0;
	}
	if (error > 0)
	{
	    ...
	    {
		case 1:
		printf("enicmp: invalid switch. stop.\n");
		return error;
		break;
		case 2:
		printf("enicmp: too many switches. stop.\n");
		return error;
		break;
		case 3:
		printf("enicmp: unknown error. stop.\n");
		return error;
		break;
	    }
	}
	if (extract == true)
	{
	    ...
	    Decomp((char *)infile, (char *)outfile, pointer, padding);
	    if (verbose == true)
	    {
		printf ("extracted: ");
		printf (infile);
	...
	else
	{
	    ...
	    output = argv[argc -1];
	    const char *outfile = output.c_str();
	    Comp((char *)infile, (char *)outfile, padding);
	    if (verbose == true)
	    {
		printf ("compressed: ");
		printf (infile);
